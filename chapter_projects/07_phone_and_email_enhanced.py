# Note to self: I wanted to enhance this slightly to account for phone & email
# labels. For example - 'General Inquiries:', 'Media Requests:', 'Office Number:',
# etc.  I was able to implement this to some extent - however, it is unclear
# to me how to account for a variable label length.  As seen in the program below,
# when copying all the text from https://nostarch.com/contactus as a test subject
# 'General Inquiries:' & 'Media Requests:' work fine only because they do not exceed
# the parameters of the defined regular expression, but also because they are
# well structured on the page. 'Help with your order:' comparatively is truncated at
# 'your order:'.  I may be able to write something so that it continues so long as it
# is NOT a newline - but again this would be dependent on a well structured input...
...
emailRegex = re.compile(r'''(
	(\w+\s\w+:\s|\w+:\s)? # looks for label, up to 2 words
...
for groups in phoneRegex.findall(text):
	...
	phoneNum = groups[1] + '-'.join([groups[2], groups[4], groups[6]])
